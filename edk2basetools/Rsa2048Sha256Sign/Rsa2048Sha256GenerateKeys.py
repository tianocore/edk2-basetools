...
if __name__ == '__main__':
    ...
    #
    try:
        Process = subprocess.Popen('%s version' % (OpenSslCommand), stdout=subprocess.PIPE,
    ...
    if args.OutputFile is not None:
        for Item in args.OutputFile:
            ...
            #
            Process = subprocess.Popen('%s genrsa -out %s 2048' % (OpenSslCommand, Item.name),
    ...
    for Item in args.PemFileName:
        ...
        #
        Process = subprocess.Popen('%s rsa -in %s -modulus -noout' % (OpenSslCommand, Item),
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
